...
class GoblinTest {
  ...
  void testMove() {
    ...
    goblin.move();
    assertEquals(initialX, goblin.x);

    // Test moving left
    goblin.movingRight = false;
    initialX = goblin.x;
    goblin.move();
    assertEquals(initialX - 4, goblin.x);

    // Test moving down
    goblin.movingDown = true;
    float initialY = goblin.y;
    goblin.move();
    assertEquals(initialY, goblin.y);

    // Test moving up
    goblin.movingDown = false;
    initialY = goblin.y;
    goblin.move();
    assertEquals(initialY - 4, goblin.y);
  }
  ...
  void testDie() {
    PApplet applet = new PApplet();
    applet.sketchPath("");
    assertTrue(goblin.getAliveStatus());

    goblin.takeDamage(100);
    assertFalse(goblin.getAliveStatus());
